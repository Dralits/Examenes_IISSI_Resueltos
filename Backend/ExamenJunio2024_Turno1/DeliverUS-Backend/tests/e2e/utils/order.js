...
async function getOrderList (owner, restaurant) {
  return (await request(await getApp()).get(`/restaurants/${restaurant.id}/orders`).set('Authorization', `Bearer ${owner.token}`).send()).body
}

export {getOrderList, getNewOrderData, getFirstProductFromRestaurant, getProductAlreadyOrdered, getNewOrderDataWithUnavailableProduct, createOrder, computeOrderPrice, checkOrderEqualsOrderData }
